...
function skuPackage() {
  return { g1Identity: {
      schemaVersion: "g1-identity-v1", candidateId: "CX-RUNTIME-001", skuPackageId: "sku:runtime:001",
    ...
    }, skuPackageId: "sku:runtime:001", businessPhase: "C1", dataRevision: 7, c1ProductPlan: { status: "facts_checked" } };
}

test("G1缺失或错绑时拒绝顶层候选副本且不进入流水线", async () => {
  for (const identity of [undefined, { candidateId: "OTHER" }]) {
    const sku = skuPackage();
    sku.g1Identity = identity;
    sku.candidateId = "CX-RUNTIME-001";
    let calls = 0;
    await assert.rejects(() => prepareC1FactKeywordRuntime({
      candidateId: "CX-RUNTIME-001", skuPackage: sku, input: input(), preparedAt: NOW
    }, { preparePipeline: async () => { calls += 1; throw new Error("unexpected pipeline call"); } }), /CANDIDATE_DRIFT/);
