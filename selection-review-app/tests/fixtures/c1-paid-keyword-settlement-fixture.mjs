...
export async function c1PaidKeywordSettlementCandidate({ candidateId = "CX-MUSIC-BOX-014", supplierSkuId = "MUSIC-WHITE" } = {}) {
  ...
  candidate.lifecycleV11.skuPackage.g1Identity.candidateId = candidateId;
  ...
  candidate.lifecycleV11.skuPackage.g1Identity.supplierSkuId = supplierSkuId;
  candidate.lifecycleV11.skuPackage.g1Identity.skuPackageId = candidate.lifecycleV11.skuPackage.skuPackageId;
  ...
  Object.assign(plan, {
    ...
    factVerificationVersion: C1_CURRENT_FACT_VERIFICATION_VERSION, factsVerifiedAt: KEYWORD_NOW,
  ...
  plan.inputSnapshots.platformSchemaRules = {
    ...
    storeRef: structuredClone(candidate.lifecycleV11.skuPackage.g1Identity.storeRef),
    schemaRevision: "ozon:music-box:1", requiredFields: [], collectedAt: KEYWORD_NOW
  };
  // Construct a current synthetic checked snapshot. These explicit licence facts
  // are independent of schema compliance and never upgrade a persisted old plan.
  const checkedSkuRevision = candidate.lifecycleV11.skuPackage.dataRevision;
  plan.revisionRefs = { sourceRevision: checkedSkuRevision - 2, resultRevision: checkedSkuRevision - 1 };
  plan.frozenInputRefs = {
    candidateId, skuPackageId: plan.identity.skuPackageId, platform: plan.identity.targetPlatform,
    storeRef: candidate.lifecycleV11.skuPackage.g1Identity.storeRef.stableStoreId,
    sourceRevision: plan.revisionRefs.sourceRevision,
    salesSnapshotId: plan.inputRefs.salesSnapshotId,
    selectedSupplySnapshotId: plan.inputRefs.selectedSupplySnapshotId,
    ownerSupplyConfirmationRef: `${plan.inputRefs.selectedSupplySnapshotId}#ownerSupplyConfirmation`,
    profitModelVersion: plan.inputRefs.profitModelVersion,
    schemaSnapshotRef: plan.inputRefs.platformSchemaEvidenceId
  };
  plan.inputSnapshots.skuRightsReview = syntheticSkuRightsReview({
    plan, sourceIdentity: candidate.lifecycleV11.skuPackage.g1Identity, reviewedAt: KEYWORD_NOW,
    brand: SYNTHETIC_LICENSED_BRAND, rights: SYNTHETIC_LICENSED_RIGHTS,
    reviewId: `rights-review:synthetic:${candidateId}:${supplierSkuId}`
  });
  plan.platformCompliance.skuRightsReview = projectC1SkuRightsReviewFacts(
    plan.inputSnapshots.skuRightsReview, `${plan.c1PlanId}#/inputSnapshots/skuRightsReview`
  );
...
export async function collectFixtureMetrics({ candidateId, skuPackage, input }) {
