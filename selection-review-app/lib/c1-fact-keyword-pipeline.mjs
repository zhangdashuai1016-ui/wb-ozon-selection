...
function resolveFactsCheckedPackage(skuPackage, preparedAt) {
  ...
  if (!["inputs_ready", "facts_checked"].includes(status)) {
    throw new Error("C1_FACT_KEYWORD_GATE_REJECTED: C1必须处于inputs_ready或facts_checked");
  }
  const rights = resolveC1SkuRightsReviewForFacts({ skuPackage, observedAt: preparedAt });
  if (rights.status !== "verified") return { skuPackage, factVerification: "not_performed", rightsGap: rights };
  if (status === "facts_checked") return { skuPackage, factVerification: "reused" };
  const verified = verifyC1ProductFacts({ skuPackage, skuRightsReview: rights.review, verifiedAt: preparedAt });
...
export async function prepareC1FactKeywordPipeline({
  ...
  if (currentSkuPackage.g1Identity?.candidateId !== candidateId) throw new Error("C1_FACT_KEYWORD_CANDIDATE_DRIFT: 候选身份不一致");
  if (resolved.rightsGap) return notReady({ candidateId, candidateRevision, skuPackage: currentSkuPackage,
    factVerification: resolved.factVerification, bindingEvidence: null, preparation: null,
    gaps: [gap(resolved.rightsGap.code, "skuPackage.c1RightsReviewRecord", "当前SKU权利声明尚未满足事实冻结条件")] });
