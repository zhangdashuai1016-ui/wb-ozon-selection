 ...
 */
export async function persistJsonThroughRealTarget(filePath, data, { fileSystem = fs, createOnly = false } = {}) {
  if (typeof createOnly !== "boolean") throw new Error("ATOMIC_JSON_CREATE_MODE_INVALID");
  const targetFile = createOnly ? filePath : await resolveRealTarget(fileSystem, filePath);
  const directory = path.dirname(targetFile);
  const mode = createOnly ? 0o600 : await targetMode(fileSystem, targetFile);
  ...
  try {
    ...
    if (createOnly) {
      // A completed, synced file becomes visible only if no prior target exists.
      // Unlike rename, link cannot overwrite a concurrent first registration.
      await fileSystem.link(tempFile, targetFile);
      replaced = true;
    ...
    if (createOnly && replaced && error.code !== "ATOMIC_JSON_DURABILITY_UNCONFIRMED") throw durabilityError(targetFile, error);
    throw error;
  }
}
