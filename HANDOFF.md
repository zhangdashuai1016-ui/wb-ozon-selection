...
1. 批准：plist 加 `SELECTION_REVIEW_GUOO_TARIFF_FILE`（指向本地配置目录里的 GUOO 表）并重启 4317（几秒）；之后主人对 3321582481 进商品页 → "找货"填 1688 链接、货价、国内运费、打包重量、长宽高 → 保存看是否过线 → 申请插件采集。
2. r13（Opus 子代理施工中）：一次调用完成创建+开始、孤儿批次可继续、新一轮排最前、旧轮次未选默认折叠、"本轮方向"改显示下一轮方向、24 小时提示只在同方向时说"同一方向"、插件状态文案改"插件已连接 · 等待采集任务"。
