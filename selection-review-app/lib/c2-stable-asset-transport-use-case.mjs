...
export async function settleC2StableAssetTransportJob({
  ...
  const snapshot = await repository.readSnapshot();
  findCandidate(snapshot, candidateId);
  const jobRecord = snapshot.runtime?.softwareJobs?.find((job) => job.jobId === jobId);
  if (!jobRecord || jobRecord.candidateId !== candidateId || jobRecord.jobType !== C2_STABLE_ASSET_TRANSPORT_JOB_TYPE) {
    throw new Error("C2_STABLE_TRANSPORT_HALF_STATE_REJECTED");
  }
  const { jobRef } = readC2StableAssetTransportSourceForSettlement(snapshot, jobRecord);
  ...
  if (status === "completed" && !existingSettlementReplay) {
    if (!(jobRecord.status === "waiting_platform" && jobRecord.externalRequestState === "in_flight") &&
        !isC2StableAssetTransportReconciledRequest(jobRecord)) {
      throw new Error("C2_STABLE_TRANSPORT_RESULT_REJECTED: 必须从已持久化in_flight收口，或仅记录原unknown终态的对账证据");
    }
  }
