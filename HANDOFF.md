...
1. 现在可直接再点一次"找一轮新品"→"确认"，然后停留在选品台几分钟不要切换页面；"刚查过同一方向"的黄色提示忽略。新一轮 20 条出现后再挑品。
2. 挑品 → 商品页"找货"填 1688 资料 → 看是否过线 → 申请插件采集。
3. r13（Opus 子代理施工中）：一次调用完成创建+开始、孤儿批次可继续、新一轮排最前、旧轮次未选默认折叠、"本轮方向"改显示下一轮方向、24 小时提示只在同方向时说"同一方向"、插件状态文案改"插件已连接 · 等待采集任务"。做完需主人批准部署。
