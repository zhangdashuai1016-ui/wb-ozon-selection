...
for (const scenario of [
  {
    ...
    failureClass: "c1-paid-keyword-system-failure", throwsUnknown: true
  },
  {
    name: "响应无法解析",
  ...
  test(`generic C1 paid keyword Worker准确收口${scenario.name}且不重试`, async () => {
    ...
    let calls = 0;
    let preparations = 0;
    const execution = runC1PaidKeywordEvidenceSoftwareJob({
      ...fixture.params,
    ...
    if(scenario.throwsUnknown)await assert.rejects(execution);
    else assert.equal((await execution).status,"committed");
    const stored = await fixture.repository.readSnapshot();
    const job = stored.runtime.softwareJobs[0];
    if(scenario.throwsUnknown)assert.equal(stored.candidates[0].executionRuntime.exceptionCase.softwareJobId,job.jobId);
...
test("generic C1 checks the lease inside each send boundary and records expired failure without sending", async () => {
  const fixture=await genericWorkerFixture();let insideTransaction=false,clockReads=0,calls=0;
  const repository={...fixture.repository,transact:mutator=>fixture.repository.transact(async document=>{
    insideTransaction=true;try{return await mutator(document);}finally{insideTransaction=false;}
  })};
  const observedAt=new Date(Date.parse(KEYWORD_NOW)+60001).toISOString();
  const outcome=await runC1PaidKeywordEvidenceSoftwareJob({...fixture.params,repository,
    serverClock:()=>{clockReads++;assert.equal(insideTransaction,true);return observedAt;},
    openApiTransport:async()=>{calls++;return genericProviderReceipt();}});
  assert.equal(outcome.result.status,'failed');assert.equal(calls,0);assert.equal(clockReads,2);
  const job=(await fixture.repository.readSnapshot()).runtime.softwareJobs[0];
  assert.equal(job.externalRequestState,'not_sent');assert.equal(job.completedAt,observedAt);
});
...
for (const { name, prepareRuntime } of [
  ...
  test(`generic C1已确认外部成功时，本地${name}不会改写外部终态`, async () => {
    ...
    const stored = await fixture.repository.readSnapshot();
    const job = stored.runtime.softwareJobs[0];
    assert.equal(stored.candidates[0].executionRuntime.exceptionCase.softwareJobId,job.jobId);
